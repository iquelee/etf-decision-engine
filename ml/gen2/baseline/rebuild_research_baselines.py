...
def render_report(inputs: dict, classified: list[dict]) -> str:
    ...
    lines += [
        ...
        "现行契约（研究侧 `portfolio.role_thresholds`；bundle 侧 `selection.role_thresholds` 已于 **WP-G2-04** 烘焙进 `gen2-rule-v2.0.1`）：",
        "",
        "```json",
        ...
        "旧 `top_quantile` 只作迁移审计字段（`top_quantile_legacy_audit`），**任何运行路径都不再读取**。",
        "**WP-G2-05R（PR #26 修订）**起 JS 运行时回退分支已删除：bundle 缺 `selection.role_thresholds`",
        "（或提供但非法）→ 本次运行 `blocked` / `status_reason=RULE_BUNDLE_INCOMPLETE`，与 Python 端同语义；",
        "旧字段的唯一出口是离线迁移助手 `deriveRoleThresholdsFromLegacy()`：**WP-G2-04 已完成一次性换算**，",
        "旧字段以 `selection.legacy_migration_audit` 留痕（`MIGRATION_AUDIT_ONLY`），",
        "因此不再存在 `role_thresholds_source=LEGACY_TOP_QUANTILE_AUDIT` 这种运行态。",
        "",
        ...
        "",
        "### F4 · 研究脚本的组合构建口径与权威账本不一致 —— **已修（WP-G2-06，PR #29）**",
        "缺陷：`build_portfolio_candidates` 曾把 CORE 权重重置为等权 `1/n`（**丢弃** `build_v2_roles` 计算的",
        "单只/cluster/广义科技上限），并以 legacy `rank`（leadership 排名）作为 `priority`；组合里也没有现金腿/防守腿。",
        "因此研究基线（归因 / 敏感性）与 `rule_v2_ab.run_v2_backtest`（含上限 + 防守）**不是同一组合口径**。",
        "",
        "修复（2026-09-14，`5ad7555`）：权重**直接沿用**角色层 `target_weight`（只做上限复核，越界抛错，",
        "绝不重算/静默缩/重置为等权）；`priority` 改由**显式注入的 selection score**给出；补现金腿 + 防守腿；",
        "且线上 `main()` 亦接入同一口径（唯一 `buildCandidatePanel()`，旧等权/旧防守链在 `main()` 中已无消费者）。",
        "本报告的重跑即基于修复后的构建口径。",
        "",
        "## 5. 边界",
        "",
        ...
        "- **WP-G2-04 已完成重新冻结**（`gen2-rule-v2.0.1` + 覆盖 bundle/JS 实现/Python 规则·候选·防守实现/",
        "  构建产物的新 lock）。本报告是**冻结之后**重跑的 B1 研究基线（即顺序表中的「冻结完成后才重跑 B1」）；",
        "  B3 Frozen OOS 排在其后。",
