...
/** 趋势评分（满分 25）— V3.8 默认：W 状态查表 */
function scoreTrend(wState) {
  const map = { W1: 25, W2: 20, W3: 15, W4: 10, W5: 5 };
  return map[wState] != null ? map[wState] : 15;
}

/** v1.1 A2：Trend 维读 trend_quality_score（0~25），与 StageFactor 职责分离 */
function scoreTrendFromQuality(trendQualityScore) {
  if (trendQualityScore == null || Number.isNaN(Number(trendQualityScore))) return 15;
  return Math.max(0, Math.min(25, Math.round(Number(trendQualityScore))));
}

/**
 * v1.1 A2 校准：机会分 Trend 维 = max(quality, W状态分)。
 * - 主升/质量高时保留 quality（不被 W 压低）
 * - W4 震荡 quality 偏低时，W 状态分托底，避免机会等级塌陷到 F
 * - overlay=broken 仅信 quality（破位段不加 W 托底）
 */
function scoreTrendForOpportunity(trendQualityScore, wState, overlay) {
  const quality = scoreTrendFromQuality(trendQualityScore);
  if (overlay === 'broken') return quality;
  return Math.max(quality, scoreTrend(wState || 'W3'));
}
 ...
 * 进攻引擎接入（2026-08-26）：支持 params.opportunity_factors 覆盖内置系数——
 * param_config 配了 opportunity_factors 即启用进攻模式；不配则行为与现状完全一致。
 * @param {number} maxStrategic 最大战略仓位（基数）
 * @param {string} grade 机会等级 A~F
 * @param {object} params 运行时阈值（可含 opportunity_factors: {A:[lo,hi],...}）
 ...
 */
function computeTargetRange(maxStrategic, grade, params = {}) {
  const custom = params.opportunity_factors;
  const table = (custom && typeof custom === 'object' && custom[grade])
    ? custom : OPPORTUNITY_FACTORS;
  const range = (table[grade] && Array.isArray(table[grade]))
    ? table[grade] : [0, 0];
...
function generateTargetPosition(ctx) {
  ...
  // 目标仓位区间（动态生成；params.opportunity_factors 可覆盖内置系数——进攻引擎）
  const range = computeTargetRange(maxStrategic, grade, params);
  // 机会系数保留区间中值（兼容旧字段；进攻模式取自定义系数）
  const factorTable = (params.opportunity_factors && typeof params.opportunity_factors === 'object' && params.opportunity_factors[grade])
    ? params.opportunity_factors : OPPORTUNITY_FACTORS;
  const factorRange = (factorTable[grade] && Array.isArray(factorTable[grade]))
    ? factorTable[grade] : [0, 0];
...
/** 判定加仓模式：横盘加仓（A+/A/B 级）/ 突破加仓 / 趋势延续加仓（实验）/ 无 */
function determineAddMode(ctx) {
  const { snapshot = {}, crowding = {}, positions = {}, params = {} } = ctx;
  ...
  // 实验（opt-in，2026-08-25）：趋势延续加仓 —— 干净急涨段（W1 + 无横盘 + 高位 + 已有仓）小步加仓。
  // 根因：scripts/diag-s7-gates.js 证实 515880 在 2025-07~09 急涨段被 chase 拦 42 天卡在 13%，
  //       横盘加仓（需横盘）/ 突破加仓（需成熟横盘）/ chase（price_position>0.8）三条路径全失效。
  // 条件（全部满足）：params.trend_follow_add===true + W1（仅最强）+ 已有仓（首仓走 firstLotTrend）
  //       + 无横盘（grade==null，A/B 走横盘加仓）+ 高位（price_position>0.8，即 chase 拦的段）
  //       + 量能非异常（v!=='V5'，防追顶部）+ 趋势上下文不向下。
  // 默认关闭 → 行为与现状完全一致；不改动 chase 既有逻辑，仅新增触发。
  if (params.trend_follow_add === true
    && w === 'W1'
    && (positions.current_position || 0) > 0
    && grade == null
    && snapshot.price_position != null && snapshot.price_position > 0.8
    && v !== 'V5'
    && trendNotDown) return '趋势延续加仓';
  return '无';
}
...
function checkAddEligibility(ctx) {
  ...
  // 实验（opt-in，2026-08-26）：首仓突破触发（first_lot_breakout）—— 针对 2024「924」型 V 型急涨
  // 冷启动踏空（诊断 §7.5：急涨段 W3 + 无横盘 + 放量，firstLotTrend/横盘加仓/突破加仓/TFA 四条路径全失效，
  // 515880 在 pp=1.00 的急涨段空仓 7 天等不到 W2）。仅补 W3 空仓这一条缝（W1/W2 已有 firstLotTrend）。
  // 条件（全部满足）：params.first_lot_breakout===true + 空仓 + W3（W4/W5 仍禁）+ 20日新高(breakout_nd)
  //   + 高位(pp>0.8) + 放量确认(V4，非 V5 防顶部) + 趋势上下文非 DOWN。首仓仍走 BUILD_FIRST_LOT_PCT
  //   小仓试探，fund/risk/sector/regime/cooldown 等其余闸门照常生效。默认关闭 → 行为零变化。
  const firstLotBreakout = params.first_lot_breakout === true
    && current <= 0
    && w === 'W3'
    && snapshot.breakout_nd === true
    && pricePosition != null && pricePosition > 0.8
    && v === 'V4'
    && snapshot.trend_context !== TREND_CONTEXTS.DOWN;
  ...
  const trend = (w === 'W1' || w === 'W2') ? 'ok'
    : (w === 'W3' && ((qualityConsol || remnantRebuild) && isShrinking || firstLotBreakout)) ? 'ok' : 'pause';
  // ② 横盘结构：C 级横盘（下跌后横盘）不进入加仓模型 → pause
  const structure = (consolidationGrade === 'C' && !firstLotTrend && !remnantRebuild && !firstLotBreakout) ? 'pause'
    : (firstLotTrend || trendAdd || remnantRebuild || firstLotBreakout) ? 'ok'
  ...
  const volume = (firstLotTrend || trendAdd || remnantRebuild || firstLotBreakout) ? 'ok'
  ...
  // 实验（2026-08-25）：趋势延续加仓模式放行（opt-in，默认关闭时 determineAddMode 永不返回该模式）
  // 实验（2026-08-26）：首仓突破触发放行（opt-in，pp>0.8 是其触发条件的一部分）
  const chase = (firstLotTrend || firstLotBreakout || pricePosition == null || pricePosition <= 0.8 || addMode === '突破加仓' || addMode === '趋势延续加仓') ? 'ok' : 'pause';
...
function computeTargetPosition(ctx, action) {
  ...
  switch (action) {
    ...
    case 'ADD': {
      ...
      } else if (addMode === '趋势延续加仓') {
        // 实验（2026-08-25）：急涨段小步长 +3（不用 W1/W2 的 +5 趋势步长），配合冷静期防追高
        step = ADD_STEP_PCT;
      } else {
...
function buildExplainChain(ctx, action, suggestedOverride) {
  ...
  // 12 加仓/减仓闸门
  const gateResult = eligibilityOverall === 'allow'
    ? '通过（8 项全过）'
    : (eligibilityOverall === 'forbid' ? '拦截（风险熔断）' : '拦截（资格未满足）');
...
module.exports = {
  scoreTrend,
  scoreTrendFromQuality,
  scoreTrendForOpportunity,
  scoreVolume,
  scoreFundamental,
  scoreCrowding,
  scoreRisk,
