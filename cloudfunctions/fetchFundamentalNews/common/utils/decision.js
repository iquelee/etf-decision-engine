...
const {
  W_STATE_LABELS, H_STATE_LABELS, V_STATE_LABELS, F_STATE_LABELS, C_STATE_LABELS, RISK_FLAG_LABELS,
  DEFAULT_PARAMS, DEFAULT_OPPORTUNITY_WEIGHTS, SCORE_MAX, TECH_SECTORS, E3_FIRST_LOT_SECTORS,
...
/** 核心仓等级白名单，非法回退 C */
function normalizeCoreGrade(grade) {
  const g = grade != null ? String(grade) : 'C';
...
function computeCoreTrade(ctx, finalTarget) {
  ...
  let grade = normalizeCoreGrade(baseGrade);
  let defense = false;
  ...
  if (fState === 'F5') {
    ...
    grade = normalizeCoreGrade(grade);
    const ratio = CORE_RATIOS[grade] != null ? CORE_RATIOS[grade] : CORE_RATIOS.C;
...
function checkAddEligibility(ctx) {
  ...
  //    W1/W2 全放行；W3（震荡）+ 高质量横盘（A/B 级）+ 缩量 → 允许小仓试探建仓；
  //    W4/W5 仍禁止；V3.9 E3：ai_network 零仓 B+ 例外（仅首仓，见 allowsW4AiNetworkFirstLot）
  ...
  // V4.2：current_position==null 为「仓位未知」，不得走空仓首仓/加仓链（区别于已知 0 仓）
  const positionUnknown = positions.current_position == null;
  const current = positionUnknown ? 0 : (Number(positions.current_position) || 0);
  const w4AiFirstLot = allowsW4AiNetworkFirstLot(ctx);
  ...
  const firstLotTrend = !positionUnknown && current <= 0 && (w === 'W1' || w === 'W2');
  const remnantRebuild = !positionUnknown && current > 0 && current <= BUILD_FIRST_LOT_PCT
    && (w === 'W1' || w === 'W2' || w === 'W3')
    && isShrinking
    && (pricePosition == null || pricePosition <= 0.8);
  const trendAdd = !positionUnknown && current > 0 && (w === 'W1' || w === 'W2') && consolidationGrade !== 'C';
  const trend = (w === 'W1' || w === 'W2') ? 'ok'
    : (w4AiFirstLot) ? 'ok'
    : (w === 'W3' && (qualityConsol || remnantRebuild) && isShrinking) ? 'ok' : 'pause';
  // ② 横盘结构：C 级横盘不进入加仓模型 → pause；E3 与实验层 decision-v39 对齐，C 级不因 w4AiFirstLot 放行
  let structure;
  if (w4AiFirstLot) {
    structure = (consolidationGrade === 'C') ? 'pause' : 'ok';
  } else {
    structure = (consolidationGrade === 'C' && !firstLotTrend && !remnantRebuild) ? 'pause'
  ...
  // ⑤ 非高位追涨（price_position ≤ 0.8 或 突破模式）；W1/W2 空仓首仓放行；E3 零仓首仓同 chase 规则
  const chase = (firstLotTrend || w4AiFirstLot || pricePosition == null || pricePosition <= 0.8 || addMode === '突破加仓') ? 'ok' : 'pause';
  const singleMax = params.single_etf_max != null ? params.single_etf_max : DEFAULT_PARAMS.single_etf_max;
  const targetMax = positions.max_position != null ? positions.max_position
    : (positions.max_strategic_position != null ? positions.max_strategic_position : singleMax);
  ...
  const positionCheck = positionUnknown ? 'pause' : 'ok';

  const items = { trend, structure, volume, fund, chase, limit, sector: sectorCheck, risk: riskCheck, cooldown: cooldownCheck, regime: regimeCheck, position: positionCheck };
  let overall = 'allow';
...
/**
 * V3.9 · E3：通信 ETF（ai_network）在 W4 且零仓时，机会分 A/B 允许首仓试探。
 * 背景：515880 春季主升段 W4 被状态机直接 WAIT，S2 踏空（见 回测报告/V3.9-候选方案对照）。
 * 范围：仅 ai_network；已有仓仍走 W4 战略减仓；闸门与 scripts/lib/decision-v39.js E3 对齐（C 级横盘仍 pause）。
 */
function allowsW4AiNetworkFirstLot(ctx) {
  const { snapshot = {}, positions = {}, etf = {} } = ctx;
  const w = snapshot.w_state || 'W3';
  if (w !== 'W4') return false;
  if (positions.current_position == null) return false;
  const current = positions.current_position || 0;
  if (current > 0) return false;
  const sector = etf.sector || '';
  if (E3_FIRST_LOT_SECTORS.indexOf(sector) < 0) return false;
  const grade = ctx.grade != null ? ctx.grade : gradeOpportunity(ctx.opportunityScore);
...
function applyHardRules(ctx) {
  ...
  if (w === 'W5') hits.push({ level: 'P1', name: 'W5', condition: '周线趋势反转', result: '战略减仓/清仓' });
    ...
    const e3 = allowsW4AiNetworkFirstLot(ctx);
    hits.push({
      level: 'P1', name: 'W4', condition: '周线趋势破坏',
      result: e3 ? '防守模式（E3 零仓可首仓试探）' : '防守模式'
    });
  }
...
function applySectorHardCap(ctx, action, suggested) {
  ...
  const techPos = portfolio.tech_position != null ? portfolio.tech_position : 0;
  const current = positions.current_position || 0;
  if (!Number.isFinite(current) || !Number.isFinite(techPos) || !Number.isFinite(suggested)) {
    return { action, suggested: Number.isFinite(suggested) ? suggested : (Number.isFinite(current) ? current : 0), hit: false };
  }
...
function runStateMachine(ctx) {
  ...
  if (!Number.isFinite(finalTarget) || !Number.isFinite(gap)) {
  ...
  // P1 周线破坏 → 战略减仓（V3.9 E3：ai_network 零仓 B+ 可继续走 Gap 建仓链）
  if (w === 'W5') return current > 0 ? 'STRATEGIC_REDUCE' : 'WAIT';
  if (w === 'W4') {
    if (current > 0) return 'STRATEGIC_REDUCE';
    if (!allowsW4AiNetworkFirstLot(ctx)) return 'WAIT';
  }
...
function computeTargetPosition(ctx, action) {
  ...
  let current = positions.current_position || 0;
  if (!Number.isFinite(current)) current = 0;
  let finalTarget = ctx.finalTarget != null ? ctx.finalTarget
    : (positions.target_std != null ? positions.target_std : (positions.target_position || 0));
  if (!Number.isFinite(finalTarget)) finalTarget = current;
  const addMode = ctx.addMode || '无';
  // 减仓地板用「当前实际核心仓」，交易仓 = 当前 − 核心仓（非建议值）
  // 保护：实际核心仓不超过当前持仓（防「建议核心仓 > 实际持仓 → 误清仓」）
  let coreRaw = positions.core_position != null ? positions.core_position : 0;
  const coreInvalid = positions.core_position != null && !Number.isFinite(positions.core_position);
  if (!Number.isFinite(coreRaw)) coreRaw = 0;
  const core = Math.min(coreRaw, current);
  const trade = Math.max(0, current - core);
  if (coreInvalid && (action === 'STRATEGIC_REDUCE' || action === 'TACTICAL_REDUCE')) return current;
  ...
  switch (action) {
    ...
    case 'STRATEGIC_REDUCE': {
      ...
      return current > core ? roundPct(core) : roundPct(current);
    }
    case 'TACTICAL_REDUCE': {
      ...
      return roundPct(Math.max(core, current - trade * ratio));
    }
    case 'BUILD':
      // 首仓分批，第一档 ≤8pct。V3.4 试过 W1/W2 提到 15：60.32%→53.54%，现金仍 65.5%，回退。
      return roundPct(Math.min(finalTarget, BUILD_FIRST_LOT_PCT));
    case 'ADD': {
      let step;
      if (addMode === '突破加仓') {
      ...
      return roundPct(Math.min(current + step, finalTarget));
    }
    case 'HOLD':
    case 'WAIT':
      return current;
 ...
 */
function buildExplainChain(ctx, action, suggestedOverride) {
  ...
  const gap = ctx.gap != null ? ctx.gap : 0;
  const eligibilityOverall = ctx.eligibilityOverall || 'allow';
  const suggestedPosition = suggestedOverride != null ? suggestedOverride : computeTargetPosition(ctx, action);
  ...
  steps.push({ step: 2, condition: `周线 ${W_STATE_LABELS[w] || ''}`, result: wResult });
  ...
  steps.push({ step: 4, condition: `基本面 ${F_STATE_LABELS[f] || ''}`, result: fResult });

  // 5 风险
  const riskResult = (risk.risk_override === true || risk.risk_flag === 'RED') ? '熔断触发'
    : (risk.risk_flag === 'YELLOW' ? '警戒（禁新增）' : '无风险熔断');
  steps.push({ step: 5, condition: `风险等级 ${RISK_FLAG_LABELS[risk.risk_flag] || '正常'}`, result: riskResult });

  // 6 拥挤度
  const premium = crowding.premium_flag || '正常';
  steps.push({ step: 6, condition: `拥挤度 ${premium} · ${C_STATE_LABELS[crowding.c_state] || ''}`, result: premium === '极端溢价' || premium === '明显溢价' ? '限制新增仓位' : '不限制' });
  ...
  // 12 加仓/减仓闸门
  const gateResult = eligibilityOverall === 'allow' ? 'PASS（8 项全通过）'
    : (eligibilityOverall === 'forbid' ? 'BLOCKED（风险熔断）'
      : (eligibilityOverall === 'pause' ? 'PAUSED（暂停新增）' : 'BLOCKED（资格未满足）'));
...
function runDecision(etf = {}, snapshot = {}, positions = {}, params = {}, extra = {}) {
  ...
  const explainChain = buildExplainChain(ctx, finalAction, suggestedPosition);
  ...
  // rule_hits：状态码 + 规则代号（兼容旧字段）；state_hits / rule_event_hits 拆分语义
  const stateHitsArr = [w, h, v, f, c].filter(Boolean);
  const ruleEventHits = ruleHits.map((r) => (r.name === 'H1V1' ? 'H1' : r.name)).filter(Boolean);
  const _seen = new Set();
  const ruleHitsArr = [];
  stateHitsArr.concat(ruleEventHits).forEach((code) => {
  ...
  return {
    ...
    state_hits: stateHitsArr,
    rule_event_hits: [...new Set(ruleEventHits)],
