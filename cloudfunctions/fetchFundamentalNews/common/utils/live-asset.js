...
/** 用 etf.sector + 当前仓位% 汇总科技/黄金/现金。快照缺字段时给总览兜底。 */
function bookFromPositions(etfs, positions) {
  const posMap = {};
  (positions || []).forEach((p) => {
    if (p && p.code) posMap[p.code] = Number(p.current_position) || 0;
  });
  let tech = 0;
  let gold = 0;
  let drug = 0;
  let total = 0;
  (etfs || []).forEach((e) => {
    const v = posMap[e.code] || 0;
    total += v;
    const sector = e.sector || '';
    if (TECH_SECTORS.indexOf(sector) >= 0) tech += v;
    else if (sector === 'gold') gold += v;
    else if (sector === 'biotech') drug += v;
  });
  return {
    tech_position: roundPct(tech),
    gold_position: roundPct(gold),
    drug_position: roundPct(drug),
    cash_ratio: roundPct(Math.max(0, 100 - total)),
    etf_total: roundPct(total)
  };
}
