'use strict';
/**
 * Deploy apiGateway with ML Shadow UI wiring (observe-only metadata).
 * Does not enable Fast Path. V3.6.1 decision logic unchanged.
 */
...
async function main() {
  ...
  const packedConst = fs.readFileSync(
    path.join(distRoot, name, 'common', 'constants.js'),
    'utf8'
  );
  const packedIndex = fs.readFileSync(
    path.join(distRoot, name, 'index.js'),
    'utf8'
  );
  if (!packedConst.includes('ml_shadow_observe')) {
    throw new Error('dist missing ml_shadow_observe — prepare-functions failed?');
  }
  if (!packedConst.includes('ML_SHADOW_SIGNAL')) {
    throw new Error('dist missing ML_SHADOW_SIGNAL — prepare-functions failed?');
  }
  if (!packedIndex.includes('buildPortfolioMlShadow') || !packedIndex.includes('ml_shadow')) {
    throw new Error('dist apiGateway missing ml_shadow UI wiring');
  }
