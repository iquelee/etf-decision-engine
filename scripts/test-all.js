...
function stageG() {
  console.log('\n== Stage G: Gen-1 Production Gates（G1-A ~ G1-Q）==');
  const r = spawnSync(NODE, [path.join(REPO, 'scripts', 'gen1-production-gates.js')], { cwd: REPO, encoding: 'utf8' });
  const ok = r.status === 0;
  const lines = (r.stdout + r.stderr).split('\n').filter(Boolean);
  lines.forEach((l) => console.log('  ' + l));
  report('G', 'Gen-1 Production Gates G1-A~Q（含 Model Candidate/Sector Contract/Persistent Latch/Economic Health/Canary Portfolio/Health Single Truth/Fail-Closed/Context Parity/Event Contract）', ok);
}
