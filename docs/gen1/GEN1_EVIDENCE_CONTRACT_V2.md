...
| 3 | `regime` | string | **`portfolio_snapshot.market_regime`**（见 §3.1） | ⛔ 不得由 Gen-1 signal lane 推导 |
...
Evidence.regime
= portfolio_snapshot.market_regime
...
**⛔ 三个候选 `regime` 字段，只允许绑定一个**：

| 字段 | 定位 |
|---|---|
| `portfolio_snapshot.market_regime` | ✅ **唯一绑定来源** |
| `decision_result.effective_market_regime` | ⛔ 不作来源 |
