...
| P0-2 | **Health DB 读取失败仍 Fail Open**：`catch → defaultHealthState()`（= OK），外层 catch 永远收不到异常 | **成立，已修** | `readHealthState` 改为**三态**：`FOUND` / `NOT_INITIALIZED` / `READ_ERROR`。`NOT_INITIALIZED → PENDING（ADVISORY 可继续 / CANARY 关闭，不落库）`；`READ_ERROR → ML_OFF（全链 fail-closed，manual_review_required，且 upsert 被跳过）`。**绝不**把异常当 OK。**G1-O（6 组，含「读异常时 upsert 必须为 0 次」）** |
