...
const SCHEMAS = [
  ...
  },
  {
    ...
    fields: {
      ...
      ml_fast_path_enabled: { type: 'boolean', required: false, desc: 'Gen-1 Fast Path 人工建议闸门' },
      ml_execution_enabled: { type: 'boolean', required: false, desc: '自动/券商执行闸门，必须为 false' },
  ...
  },
  {
    ...
    fields: {
      ...
      v361_baseline_stage: { type: 'string', required: false, desc: 'V3.6.1 基线阶段（Gen-1 Advisory 对照）' },
      v361_baseline_target: { type: 'number', required: false, desc: 'V3.6.1 基线目标仓位%' },
      v361_baseline_action: { type: 'string', required: false, desc: 'V3.6.1 基线动作' },
      gen1_effective_stage: { type: 'string', required: false, desc: 'Gen-1 Advisory 有效阶段' },
      gen1_advisory_target: { type: 'number', required: false, desc: 'Gen-1 Advisory 主建议目标仓位%' },
      gen1_advisory_action: { type: 'string', required: false, desc: 'Gen-1 Advisory 主建议动作' },
      ml_advisory_effective: { type: 'boolean', required: false, desc: '本次是否由 Gen-1 影响人工主建议' },
      ml_model_id: { type: 'string', required: false, desc: '生效/观察模型 ID' },
      ...
      gen1_candidate_hash: { type: 'string', required: false, desc: '候选输入稳定哈希（~ 分隔 + 字典序 + | + sha256）' },
      gen1_adopted: { type: 'boolean', required: false, desc: 'Gen-1 候选是否被采纳进生产计算（GE-02 恒 false）' },
      gen1_reject_reason_code: { type: 'string', required: false, desc: '未采纳原因码' },
      gen1_safety_core_adjust_reason: { type: 'string', required: false, desc: 'V3 采纳后又被裁剪的原因' },
      gen1_guarded_baseline_stage: { type: 'string', required: false, desc: 'baseline 阶段' },
      gen1_guarded_effective_stage: { type: 'string', required: false, desc: 'guarded 重跑使用的阶段（S4）' },
      gen1_guarded_baseline_target: { type: 'number', required: false, desc: 'baseline target' },
  ...
  {
    name: COLLECTIONS.ML_SHADOW_SIGNAL,
    fields: {
      date: { type: 'string', required: true, desc: '信号日 YYYY-MM-DD' },
      source_trade_date: { type: 'string', required: false, desc: '推理所用 EOD 交易日 YYYY-MM-DD' },
      code: { type: 'string', required: true, desc: 'ETF 代码' },
      stage: { type: 'string', required: false, desc: '趋势阶段' },
      ml_probability: { type: 'number', required: false, desc: '原始概率' },
      calibrated_probability: { type: 'number', required: false, desc: '校准概率' },
      ml_fast: { type: 'boolean', required: false, desc: '是否 ML 快速意图' },
      rule_gate: { type: 'string', required: false, desc: 'PERMIT/BLOCK' },
      permission_hit: { type: 'boolean', required: false, desc: '许可命中' },
      fast_path_would_trigger: { type: 'boolean', required: false, desc: '若开闸是否会触发' },
      signal_status: { type: 'string', required: false, desc: 'NO_OPPORTUNITY/OBSERVED/CANDIDATE/BLOCKED/DEGRADED' },
      ml_counterfactual_target: { type: 'number', required: false, desc: '反事实目标仓' },
      v361_target: { type: 'number', required: false, desc: '正式目标仓（记录）' },
      delta_target: { type: 'number', required: false, desc: '差额' },
      decision_hash: { type: 'string', required: false, desc: '决策哈希' },
      feature_schema_hash: { type: 'string', required: false, desc: '冻结特征 schema 哈希' },
      model_id: { type: 'string', required: false, desc: '规范模型 ID' },
      ml_model_id: { type: 'string', required: false, desc: '模型 ID' },
      ml_effective: { type: 'boolean', required: false, desc: '底层信号是否直接写入交易决策（保持 false）' },
      ml_advisory_enabled: { type: 'boolean', required: false, desc: '是否允许 Gen-1 Advisory' },
      advisory_effective: { type: 'boolean', required: false, desc: '本行是否满足 Advisory 候选条件' },
      ml_execution_enabled: { type: 'boolean', required: false, desc: '自动/券商执行，必须为 false' },
   ...
   */
  {
    ...
    fields: {
      ...
      ml_effective: { type: 'boolean', required: false, desc: 'legacy alias，单向派生自 gen1_guarded_effective_active' },
      gen1_authority: { type: 'string', required: false, desc: 'OFF/SHADOW/ADVISORY/CANARY/GUARDED_EFFECTIVE/PRODUCTION（PRODUCTION 不可达）' },
      ...
      // ---- WP-G1-GE-02：Guarded Effective 运行状态（章程 §6.2 + 封印可观测）----
      gen1_guarded_effective_authorized: { type: 'boolean', required: false, desc: 'Key 1 成立' },
      gen1_guarded_effective_evidence_allowed: { type: 'boolean', required: false, desc: 'Key 3 成立（PASS 且 evidence_positive 且事件 >= 30）' },
      gen1_guarded_effective_health_allowed: { type: 'boolean', required: false, desc: '运行时 Health 门成立（health=OK 且 gate_status 显式 ACTIVE）' },
      ...
      gen1_guarded_evidence_seal_reason_code: { type: 'string', required: false, desc: 'Evidence Seal 未通过原因码（含 EVIDENCE_SEAL_NOT_POSITIVE）' },
      gen1_guarded_evidence_independent_events: { type: 'number', required: false, desc: '独立事件数（证据门硬前置 >= 30）' },
      gen1_guarded_selector_source: { type: 'string', required: false, desc: '权威结果来源（GE-02 恒 BASELINE）' },
