...
/**
 * 计算 ATR(n)（Wilder 简化：前 n 个 TR 均值，返回最新值）。
...
function detectBreakout(sidewayDays, highArr, lastClose, params = {}) {
  const minDays = params.sideway_days_min != null ? params.sideway_days_min : DEFAULT_PARAMS.sideway_days_min;
  if (sidewayDays < minDays || !highArr || lastClose == null) return false;
...
function detectDState(dailyBars, params = {}) {
  if (!dailyBars || dailyBars.length < 20) return 'D5';
...
function computeSnapshot(dailyBars, params = {}, opts = {}) {
  ...
  // 20 日新高突破（不依赖横盘，只增字段，2026-08-26）：供「首仓突破触发」实验旗标（first_lot_breakout）
  // 使用——V 型急涨段横盘为 0、breakout 恒 false，需独立的 N 日新高语义。默认无消费方，零行为影响。
  const priorClose20 = closeArr.length > 20 ? max(closeArr.slice(-21, -1)) : null;
  const breakoutNd = priorClose20 != null && lastClose > priorClose20;
